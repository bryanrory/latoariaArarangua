// Latoaria Ararangua - Premium Scripts
(function () {
  'use strict';
  ...
  async function loadSiteSettings() {
    try {
      ...
      if (data.whatsapp) {
        ...
        var waUrl = 'https://wa.me/' + waNumber + '?text=' + encodeURIComponent('Olá! Vi o site da Latoaria Ararangua e gostaria de solicitar um orçamento.');
        ...
        if (data.horario_funcionamento) {
          contatoGrid.innerHTML +=
            '<div class="contato-item" data-anim>' +
              ...
              '<h3>Horario</h3><p>' + data.horario_funcionamento + '</p>' +
            '</div>';
        }
      }
  ...
  async function loadAmbientes() {
    try {
      ...
      if (footerServicos) {
        var servicosHtml = '<h4>Servicos</h4>';
      ...
      data.forEach(function (env, index) {
        ...
        card.innerHTML =
          ...
          '<div class="card-info">' +
            '<h3>' + env.name + '</h3>' +
            '<p>' + (env.description || 'Servico especializado') + '</p>' +
          '</div>';
      ...
      if (totalCount > 6) {
        ...
        btnWrap.innerHTML = '<a href="ambientes.html" class="btn btn-primario"><span>Ver Todos os Trabalhos</span><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>';
